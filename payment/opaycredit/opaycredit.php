...
class plgVmPaymentOPayCredit extends vmPSPlugin {
    ...
    function plgVmConfirmedOrder($cart, $order) {
        ...
        if ($order) {
            try {
                ...
                $oPayment->ServiceURL = ($this->_currentMethod->test_mode == 'yes' ? 'https://payment-stage.opay.tw/Cashier/AioCheckOut/V4' : 'https://payment.opay.tw/Cashier/AioCheckOut/V4');
                ...
                $oPayment->Send['TradeDesc'] = "OPay_VirtueMart_Plugin";
    ...
    /**
     * Receive and process with response data from OPay.(必要)
     * @param type $html
     * @return boolean
     */
    function plgVmOnPaymentResponseReceived(&$html) {
        ...
        $oPayment = new AllInOne();
        $oPayment->ServiceURL = ($this->_currentMethod->test_mode == 'yes' ? 'https://payment-stage.opay.tw/Cashier/QueryTradeInfo/V4' : 'https://payment.opay.tw/Cashier/QueryTradeInfo/V4');
        $oPayment->HashKey = $this->_currentMethod->hash_key;
        $oPayment->HashIV = $this->_currentMethod->hash_iv;
        ...
        try {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                if ($deTradeAmount == $deTotalAmount) {
                    ...
                    throw new Exception("Compare '$szOrderID' Order Amount Fail.");
                }
            } else {
                throw new Exception("Order('$szOrderID') Not Found at OPay.");
            }
        } catch (Exception $e) {
            // 背景訊息
